...
being handled by DQB while transforming your filters into a LINQ expression. 

DQB also can handle ~null~ values as well. DQB can handle both filtering by sql 
~null~ keyword and and "null" string value. In order to select between these 
choices IsNullValueString property of DynamicQueryOptions.
...
An example usage of ~Filter~ using ~null~ as string: 
...
//  LINQ Translation: myCollection.Where(x => x.name == "null");
#+END_SRC
